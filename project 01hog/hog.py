...
def pig_pass(player_score, opponent_score):
    ...
    assert type(player_score)==type(opponent_score)==int,'得分必须是整数'
...
def play(strategy0, strategy1, score0=0, score1=0, dice=six_sided,
    ...
    while score0<goal and score1<goal:
        ...
        #零号玩家开玩
            if score0<goal:
                score0+=take_turn(strategy0(score0,score1), score1, dice)#第一次掷骰子
                say=say(score0,score1)
                while extra_turn(score0, score1) and score0<goal:
                    score0+=take_turn(strategy0(score0,score1), score1, dice)#连续继续掷骰子嘛
                    say=say(score0,score1)
            else:
                return score0 and score1
        else:
            if score1<goal:
                ...
                while extra_turn(score1,score0)and score1<goal:
                    score1+=take_turn(strategy1(score1,score0), score0, dice)#连续继续掷骰子嘛
                    say=say(score0,score1)
            else:
                return score0 and score1
        who=other(who)
...
def announce_highest(who, last_score=0, running_high=0):
    ...
    def say(score0,score1):
        if who==0:
            gained_score,last_score_update=score0-last_score,score0
        else:
            gained_score,last_score_update=score1-last_score,score1
        if gained_score>running_high:
            print(gained_score,'point(s)! The most yet for Player',who)
            running_high_update=gained_score
        else:
...
def extra_turn_strategy(score, opponent_score, cutoff=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 11
    def zero_score(score, opponent_score):
        score_now=score+free_bacon(opponent_score)
        if extra_turn(score_now,opponent_score):
            return True
        else:
            return False
    if bacon_strategy(score, opponent_score, cutoff, num_rolls)==0 or zero_score(score,opponent_score):
        k=0
...
def final_strategy(score, opponent_score,cutoff=8, num_rolls=6):
    ...
    能多扔一次就选择能多一次的扔法，不然选择风险最小的期望最高的扔法
    """
    # BEGIN PROBLEM 12
    if not extra_turn_strategy(score, opponent_score, cutoff, num_rolls):
        k=0
    else:
        k=8
    return k
