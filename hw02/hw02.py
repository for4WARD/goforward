...
def change_plus(n):
    return num_eights(n) or not n%8
def pingpong(n):
...
def missing_digits(n):
    ...
    True
    """
    def missing_digits_helper(n, last, result=0):
        if n <10 :
            return result
        else:
            all_but_last=n//10
            if  last - all_but_last%10 > 1:
                return missing_digits_helper(all_but_last,all_but_last%10, result + (last - all_but_last%10 -1))
            else:
                return missing_digits_helper(all_but_last,all_but_last%10 , result)
...
def make_anonymous_factorial():
    ...
    True
    """
    return lambda b:(lambda g,b:g(g,b))(lambda g,b : b*g(g,b-1) if b>1 else 1,b)
